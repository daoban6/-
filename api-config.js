...
function safeLocalStorageSet(key, value) {
  try {
    ...
    console.warn('localStorage access blocked, using sessionStorage');
    try {
...
function getCurrentUser() {
  const userStr = safeLocalStorageGet('currentUser');
...
const MaterialAPI = {
  ...
  updateQuantityBatch: async (base64Data) => {
    const response = await fetch(`${API_BASE_URL}/materials/update-quantity`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ base64Data })
    });
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || '批量更新库存失败');
    }
    return result;
  }
};
...
const OperationLogAPI = {
  getAll: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const response = await fetch(`${API_BASE_URL}/operation-logs?${queryString}`, {
      method: 'GET',
      headers: getHeaders()
    });
    const result = await response.json();
  ...
  create: async (logData) => {
    ...
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || '创建操作日志失败');
    }
    return result;
  },
  delete: async (id) => {
    const response = await fetch(`${API_BASE_URL}/operation-logs/${id}`, {
      method: 'DELETE',
      headers: getHeaders()
    });
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || '删除操作日志失败');
    }
    return result;
  },
  clear: async () => {
    ...
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || '清空操作日志失败');
    }
    return result;
  }
};
